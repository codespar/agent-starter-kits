...
**What the second shape costs, and it is a real cost.** Nothing binds the SET. Each line is attested by its own `items_hash`, but a person who approved four lines while the agent ran three leaves a bundle that does not say a fourth existed. The skipped line appears in the tool's own report, which lives in `transcript.jsonl` — the model's conversation, not the attested record. Issue #21 closes this with a `batch_hash` over the ordered lines, carried in every artifact of the batch, with `inspect` showing "3 of 4"; until it lands, an agent of this shape should not claim more than "each line is attested".
