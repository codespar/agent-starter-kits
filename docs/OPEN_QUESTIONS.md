...
Found while typing, on the kit's side: the charge read's 409 is `issuance_unconfirmed | charge_reference_ambiguous`, and `CodeSparChargeRail.read` maps any 409 to `in_flight`. A reference that matched more than one charge would read as a charge still issuing. The kit's references are its own attempt ids, so this has not been seen, but the mapping is wrong and is not changed here.
