 ...
 * turns, the decision the approver gives, what the counterparty does with what
 * the run produced, the hooks that make the world change mid-run (revocation,
 * an uncertain rail answer) and what the trail must look like at the end. The
 * model's outputs come from a recorded transcript, so the same file feeds the
 * CI, the live demo and the docs.
 */
...
const TurnSchema = z
  .object({
    ...
    /** `await-payer` only: what the payer does with what this turn issued — pays during the turn, pays only after the last turn (`late`), lets it expire (stub-only), or nothing. */
...
const ExpectSchema = z
  .object({
    ...
    /** Messages the counterparty received about outcomes: one per outcome, never two. */
...
export function scenariosDir(agent: Agent): string {
  return join(agent.dir, "scenarios");
}

export function listScenarios(agent: Agent): string[] {
  return readdirSync(scenariosDir(agent))
    .filter((f) => f.endsWith(".json"))
    .map((f) => basename(f, ".json"))
    .sort();
}

export function loadScenario(agent: Agent, name: string): Scenario {
  const path = join(scenariosDir(agent), `${name}.json`);
  if (!existsSync(path)) throw new Error(`unknown scenario ${name}; available: ${listScenarios(agent).join(", ")}`);
...
export async function runScenario(agent: Agent, scenario: Scenario, options: RunScenarioOptions): Promise<ScenarioRun> {
  ...
  const stateDir = options.stateDir ?? mkdtempSync(join(tmpdir(), `${agent.slug}-${scenario.name}-${options.mode}-`));
  const s: Setup = setup(agent, {
    mode: options.mode,
    rail,
    provider: "replay",
    transcript: resolve(scenariosDir(agent), scenario.transcript),
  ...
  const approver = { id: s.kit.labels.evalUser, channel: "terminal" };
    ...
    for (const turn of scenario.turns) {
      ...
      const loop = s.makeLoop(runtime, async (execution: Execution) => {
        ...
        const decided = await handleExecution(execution, {
          ...
          ...(awaitsPayer ? { simulatePayer: turn.payer === "pays", waitSeconds: options.waitSeconds ?? (rail === "api" ? 60 : 0) } : {}),
        });
