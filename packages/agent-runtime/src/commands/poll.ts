/**
 * `codespar-agent poll [--wait <seconds>] [--simulate-payer] [--json]`: keeps
 * looking at every receivable still waiting for its payer until it closes or
 * the wait runs out. This is the loop-closer for a terminal that has no
...
export async function poll(agent: Agent, argv: string[]): Promise<number> {
  ...
  const tell = (l: string) => void (json ? stderr : stdout).write(l + "\n");
  const s = setup(agent, { say, runId: `run_poll_${Date.now().toString(36)}` });
  try {
    const results = [];
    for (const open of s.engine.list({ state: "executing" })) {
      if (open.reason !== "awaiting_settlement") continue;
      const r = await waitForPayer(open.id, { setup: s, approver: { id: s.kit.labels.defaultUser, channel: "terminal" }, say, tell, waitSeconds: wait, simulatePayer });
