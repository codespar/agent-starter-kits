...
2. **A batch is a loop of executions, one per line.** Never fold a batch back into one multi-item execution to "save a round trip". A multi-item execution settles only when every attempt settles and its dispatch loop stops at the first refusal, so one bad supplier would strand every payee after it. `src/modules/batch-payout.ts` says why, at length; read it before changing the loop.
