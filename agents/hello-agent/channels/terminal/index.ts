...
import { formatBRL } from "../../src/month.js";
...
export async function interactive(options: TerminalOptions & { runtime: import("@codespar/agent-core").AgentRuntime }): Promise<void> {
  ...
  say(`hello-agent ${setup.manifest.manifest.version} — approval: ${setup.mode} — trilho: ${setup.railKind} — mandato ${setup.mandate.id}`);
  say(`run ${setup.runId} — bundle em ${relative(process.cwd(), setup.bundle.dir)}`);
  say('Pergunte sobre as contas do mes ("quais contas vencem em outubro?"). Ctrl+D ou "sair" encerra.');
  for (;;) {
    let text: string;
    try {
