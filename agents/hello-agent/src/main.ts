 ...
 * `npm start -- --input "quais contas vencem em outubro?"`  one turn, no prompt
 * `npm start -- --scenario happy-path`                     a scenario pack
 *
...
import { readDotEnv, resolveProvider, setup, type ProviderKind } from "./setup.js";
...
export function parseArgs(argv: string[]): Args {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (a === "--input") args.input = next();
      ...
      const v = next();
      if (v !== "human") throw new Error("--mode must be human (this agent has one mode)");
      args.mode = v;
...
const USAGE = `hello-agent
  npm start                                                 interactive terminal
  npm start -- --input "quais contas vencem em outubro?"    one turn (--json for machine output)
  npm start -- --scenario <name>                            run a scenario pack (see scenarios/)
options: --provider anthropic|replay  --transcript <file>  --user <id>  --json`;
...
export async function main(argv = process.argv.slice(2)): Promise<number> {
  ...
  // Sandbox by construction: this agent calls no API, and a live key still dies here, before anything else runs.
  const key = process.env["CODESPAR_API_KEY"];
  if (key && key !== "csk_test_your_key_here" && !isTestKey(key)) {
  ...
  const s = setup({ mode: args.mode, provider, transcript, say });
  ...
  try {
    ...
    const payload = {
      ...
      executions: executions.map((e) => ({ id: e.id, state: e.state, reason: e.reason ?? null, total_minor: e.total })),
