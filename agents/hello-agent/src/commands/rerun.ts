...
const s = setup({ mode, rail: "stub", provider: "replay", transcript: transcriptPath, runId: `${runId}_rerun_${Date.now().toString(36)}`, stateDir: mkdtempSync(join(tmpdir(), "hello-rerun-")), stubRail: { refusePayees: refusedPayees }, say });
try {
  let decisionIndex = 0;
