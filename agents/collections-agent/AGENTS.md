...
8. **The cycle closes on the state machine, not on prose.** `commerce.charge.paid` settles; `expired` and `cancelled` fail. By poll (`GET /v1/charges/{id}`) or by webhook (`packages/agent-runtime/src/webhook.ts`, `npm run webhook`), once and only once. A duplicate event never sends the debtor a second message.
...
12. **Collection has rules** (hours, secrecy of the debt, no embarrassment, LGPD). The prompt codifies them, the envelope enforces the hours at the gate, and the channel enforces them again on every outbound message (`packages/agent-runtime/src/channels/rules.ts`): nothing outside the collection hours, nothing to a contact other than the bound one, nothing naming another debtor's agreement, no CPF or CNPJ in a message. Do not loosen any of them, and do not add a path that sends a message without going through `channel.send`.
13. **A channel is the runner's, the conversations are this agent's.** `channels/whatsapp/<name>.json` names the contact, the agreement and the debtor's turns; `agent.yaml` declares the channel; `npm run check` fails if the two disagree in either direction. Never put a real phone number or a real document in one — the shipped conversation is a fixture, like `src/agreements.ts`.
14. **The official WhatsApp backend is never called from a test or the CI.** The credentials are absent by default and commented out in `.env.example`. What the tests drive are the pure functions (signature, webhook parse, handshake, request builder); a test that reaches Meta is a bug, not coverage.
