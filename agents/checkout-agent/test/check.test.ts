...
function copyAgent(): string {
  const dir = mkdtempSync(join(tmpdir(), "checkout-check-"));
...
const tools = (dir: string) => JSON.parse(readFileSync(join(dir, "tools.json"), "utf8")) as { meta_tools: Array<{ name: string; effect: string }>; local_tools: Array<{ name: string; effect: string }> };
...
describe("npm run check: the manifest is the index", () => {
  ...
  it("cart_update is declared `state`, and declaring it a read is not how to make the file pass (checkout §9.8)", () => {
    expect(tools(AGENT_DIR).local_tools.find((t) => t.name === "cart_update")?.effect).toBe("state");
    const dir = copyAgent();
    const t = tools(dir);
    writeFileSync(join(dir, "tools.json"), JSON.stringify({ ...t, meta_tools: t.meta_tools.map((x) => ({ ...x, effect: "state" })) }));
    expect(codes(dir)).toContain("tools_invalid");
  });
