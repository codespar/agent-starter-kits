 ...
 * never repeated; stale open ones expire.
 */
...
try {
  ...
  for (const stuck of s.engine.list({ state: "executing" })) {
    ...
    resumed.push({ id: closed.id, state: closed.state, receipt_ids: closed.outcomes.filter((o) => o.receipt_id).map((o) => o.receipt_id) });
    say(`${closed.id}: executing -> ${closed.state}`);
  }
  ...
  for (const e of open) say(`${e.id}: still ${e.state}; decide it with npm start`);
  ...
  process.exit(resumed.some((r) => r.state === "executing") ? 3 : 0);
} finally {
  s.close();
}
